...
4. En Supabase abre **Project Settings → API** y copia:
   - Project URL
   - `service_role` key

La `service_role` es secreta y solo debe ir en Vercel.
...
`vercel.json` incluye un cron cada hora:
...
Los avisos no son al segundo exacto porque el cron revisa cada hora. Se envían aproximadamente:

- 24 horas antes.
- 1 hora antes.
- Al abrir el check.

Revisa los límites de Cron de tu plan de Vercel. También puedes llamar manualmente al endpoint enviando:
